  ...
  const body = `
      ...
      <div class="card max-w-xl mx-auto">
        <form id="github-access-form" class="space-y-4" novalidate>
          ...
          <div>
            <label for="github-username" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">GitHub username <span class="font-normal text-slate-400">(optional)</span></label>
            <input id="github-username" name="githubUsername" type="text" autocomplete="username" maxlength="39" placeholder="your-github-username" class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-transparent px-4 py-3 text-slate-900 dark:text-slate-100 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20">
          </div>
          <div>
        ...
        (function () {
          ...
          form.addEventListener('submit', async function (event) {
            ...
            if (githubUsername && !/^[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,37}[a-zA-Z0-9])?$/.test(githubUsername)) {
              setMessage('Enter a valid GitHub username, or leave it blank.', 'error');
              return;
            }
