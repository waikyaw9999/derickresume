...
<head>
    ...
    <style>
      ...
      .blog-prose table { width:100%; border-collapse:collapse; margin:1rem 0; font-size:0.95rem; }
      .blog-prose th, .blog-prose td { text-align:left; padding:0.5rem 0.75rem; border-bottom:1px solid var(--border); }
      .blog-prose code { font-size:0.875em; padding:0.1rem 0.35rem; border-radius:0.35rem; background:rgba(79,70,229,0.08); }
      .dark .blog-prose code { background:rgba(165,180,252,0.12); }
    ...
    <header class="border-b border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md sticky top-0 z-50">
      <div class="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
        ...
        <nav class="flex flex-wrap items-center justify-end gap-3 sm:gap-4 text-sm font-medium" aria-label="${escapeHtml(navLabel)}">
          ...
          <a href="${escapeHtml(LINKEDIN_URL)}" target="_blank" rel="noopener noreferrer me" class="text-slate-600 hover:text-indigo-600 dark:text-slate-300 dark:hover:text-indigo-400">LinkedIn</a>
...
function renderBlogIndex() {
  ...
  const blogLd = {
    ...
    blogPost: posts.map((post) => ({
      ...
      author: { '@type': 'Person', name: AUTHOR_NAME, url: SITE_URL, sameAs: [LINKEDIN_URL] },
    })),
  };
