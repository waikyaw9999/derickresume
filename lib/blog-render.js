...
<body class="antialiased">
    ...
    <header class="border-b border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md sticky top-0 z-50">
      <div class="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
        <a href="/${brandLabel === 'Showcase' ? 'showcase' : 'blog'}" class="font-display text-xl font-bold text-indigo-600 dark:text-indigo-400">JuneKo<span class="text-slate-900 dark:text-white">.online</span> ${escapeHtml(brandLabel)}</a>
        <nav class="flex items-center gap-4 text-sm font-medium" aria-label="${escapeHtml(navLabel)}">
    ...
    <footer class="border-t border-slate-200 dark:border-slate-800 py-8 text-center text-sm text-slate-500 dark:text-slate-400">
      <p>&copy; ${new Date().getUTCFullYear()} ${escapeHtml(AUTHOR_NAME)}. <a class="text-indigo-600 dark:text-indigo-400 font-medium" href="/showcase">Showcase</a> · <a class="text-indigo-600 dark:text-indigo-400 font-medium" href="/blog">Mini Blog</a></p>
    </footer>
</body>
</html>`;
}
